...
ffmpeg -report \
       ...
       -filter_complex \
	       "[0]colorkey=color=0xef80e0:similarity=0.5:blend=0.0[a]; 
	          [a]split[a1][a2];
	        [1]yaepblur[b]; 
	          [b]split[b1][b2]; 
	            [a1]format=nv12,hwupload[hw0]; 
	            [b1]format=nv12,hwupload[hw1]; 
	              [hw1][hw0]vstack_vaapi,scale_vaapi=h=2304:force_original_aspect_ratio=1[c]; 
	            [b2][a2]overlay,format=nv12,hwupload[d]; 
	              [c][d]hstack_vaapi,scale_vaapi=w=4096:force_original_aspect_ratio=1[e]; 
  	                [e]scale_vaapi[vout]" -t $duration \
...
#       -filter_complex "[0]colorkey=color=0xef80e0:similarity=0.5:blend=0.0[a]; [a]split[a1][a2]; [1]yaepblur[b]; [b]split[b1][b2]; [a1]format=nv12,hwupload[hw0]; [b1]format=nv12,hwupload[hw1]; [hw1][hw0]vstack_vaapi,scale_vaapi=h=2304:force_original_aspect_ratio=1[c]; [b2][a2]overlay,format=nv12,hwupload[d]; [c][d]hstack_vaapi,scale_vaapi=w=4096:force_original_aspect_ratio=1[e]; [e]scale_vaapi[vout]" -t $duration \
